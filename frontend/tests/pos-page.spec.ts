...
test('permite registrar QR pendiente y cambiar entre todos los métodos reales', async ({ page }) => {
  const sale = { idVenta: 93, tipoVenta: 'PRESENCIAL', modalidadComercial: 'MINORISTA', estado: 'PAGADA', total: '150.00', numeroComprobante: 'PRE-000093', sucursal: branch, cajero: cashier, turno: { idTurno: 30 }, detalles: [{ idDetalleVenta: 2, cantidad: 1, precioUnitario: '150.00', subtotal: '150.00', variante: variant }] }
  await page.route('**/pos/turnos/actual', route => route.fulfill({ json: shift }))
  await page.route('**/inventory/inventarios', route => route.fulfill({ json: [inventory] }))
  await page.route('**/catalog/variantes', route => route.fulfill({ json: [variant] }))
  await page.route('**/sales/presencial', route => route.fulfill({ status: 201, json: sale }))
  await page.route('**/payments', async route => {
    expect(route.request().postDataJSON()).toMatchObject({ idVenta: 93, metodo: 'QR', monto: 150, referenciaPasarela: 'QR-00093' })
    await route.fulfill({ status: 201, json: { idPago: 103, idVenta: 93, metodo: 'QR', monto: '150.00', estado: 'PENDIENTE', referenciaPasarela: 'QR-00093' } })
  })
  await openPos(page)
  await page.getByLabel('Buscar producto o SKU').fill('aurora')
  await page.getByRole('button', { name: 'Agregar' }).click()
  await page.getByRole('button', { name: 'Continuar al cobro' }).click()
  const dialog = page.getByRole('dialog', { name: 'Cobro' })
  const method = dialog.getByLabel('Método de pago')
  await expect(method.locator('option')).toHaveText(['EFECTIVO', 'QR', 'TARJETA', 'TRANSFERENCIA', 'CONTRAPAGO'])
  for (const option of ['QR', 'TARJETA', 'TRANSFERENCIA', 'CONTRAPAGO']) {
    await method.selectOption(option)
    await expect(method).toHaveValue(option)
  }
  await method.selectOption('QR')
  await dialog.getByLabel('Referencia o comprobante').fill('QR-00093')
  await dialog.getByRole('button', { name: 'Completar venta' }).click()
  await expect(page.getByText('Pago pendiente')).toBeVisible()
  await expect(page.getByText('PENDIENTE', { exact: true })).toBeVisible()
  await expect(page.getByText('QR-00093')).toBeVisible()
})

test('checkout con métodos de pago es responsive en móvil', async ({ page }) => {
  await page.setViewportSize({ width: 375, height: 812 })
  await page.route('**/pos/turnos/actual', route => route.fulfill({ json: shift }))
  await page.route('**/inventory/inventarios', route => route.fulfill({ json: [inventory] }))
  await page.route('**/catalog/variantes', route => route.fulfill({ json: [variant] }))
  ...
  await expect(page.getByRole('dialog', { name: 'Cobro' })).toBeVisible()
  await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
})
