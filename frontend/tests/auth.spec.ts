...
test('login envía credenciales y token, restaura sesión y permite logout', async ({ page }) => {
  ...
  await page.route('**/catalog/variantes', route => route.fulfill({ json: [] }))
  await page.route('**/catalog/categorias', route => route.fulfill({ json: [] }))
  await page.route('**/inventory/inventarios', route => route.fulfill({ json: [] }))
  await page.route('**/cart/me', route => route.fulfill({ json: { idCarrito: 1, estado: 'ACTIVO', detalles: [] } }))
