...
export default function AuthForm({ mode }: { mode: 'login' | 'register' }) {
  ...
  return <AuthLayout>
    <h1>{isRegister ? 'Tu estilo empieza aquí' : 'Bienvenida de nuevo'}</h1>
    <p className={styles.subtitle}>{isRegister ? 'Crea tu cuenta y descubre Fashionstore.' : 'Inicia sesión para descubrir la tienda o acceder a tu espacio de trabajo.'}</p>
    ...
    <p className={styles.register}>{isRegister ? '¿Ya tienes cuenta?' : '¿No tienes cuenta?'} <Link to={isRegister ? '/login' : '/register'} className={styles.link}>{isRegister ? 'Inicia sesión' : 'Regístrate'}</Link></p>
  </AuthLayout>
}
