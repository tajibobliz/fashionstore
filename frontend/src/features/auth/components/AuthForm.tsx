...
export default function AuthForm({ mode }: { mode: 'login' | 'register' }) {
  ...
  return <AuthLayout>
    <h1>{isRegister ? 'Tu negocio empieza aquí' : 'Bienvenida de nuevo'}</h1>
    <p className={styles.subtitle}>{isRegister ? 'Regístrate como ENCARGADO para preparar tu empresa, sucursales y puntos de venta.' : 'Inicia sesión para descubrir la tienda o acceder a tu espacio de trabajo.'}</p>
    ...
    <p className={styles.register}>{isRegister ? '¿Ya tienes cuenta?' : '¿No tienes cuenta?'} <Link to={isRegister ? '/login' : '/register'} className={styles.link}>{isRegister ? 'Inicia sesión' : 'Registra tu negocio'}</Link></p>
  </AuthLayout>
}
