...
import type { CatalogItem, CreateProductoRequest, Producto, TipoTryOn, Variante } from '../../types/catalog'
...
export default function CatalogManagement() {
  ...
  return <section className={dashboardStyles.content}>
    ...
    {national && <div className={styles.grid}><MasterPanel title="Categorías" label="Nueva categoría" items={categories.data ?? []} onCreate={nombre => master.mutate({ kind: 'category', body: { nombre } })} /><MasterPanel title="Tallas" label="Nueva talla" items={sizes.data ?? []} onCreate={nombre => master.mutate({ kind: 'size', body: { nombre } })} /><MasterPanel title="Colores" label="Nuevo color" items={colors.data ?? []} onCreate={nombre => master.mutate({ kind: 'color', body: { nombre } })} /></div>}
    {national && <div className={styles.grid}>
    ...
    <div className={styles.tables}><DataTable title="Productos" headers={national ? ['Producto','Categoría','Minorista','Mayorista','Mínimo','Vestidor','Acciones'] : ['Producto','Categoría','Minorista','Mayorista','Mínimo','Vestidor']} rows={(products.data ?? []).map(item => national ? [item.nombre,item.categoria?.nombre ?? '—',money(item.precio),money(item.precioMayorista),String(item.cantidadMinimaMayorista ?? '—'),tryOnBadge(item),<button type="button" onClick={() => setEditing(item)}>Editar</button>] : [item.nombre,item.categoria?.nombre ?? '—',money(item.precio),money(item.precioMayorista),String(item.cantidadMinimaMayorista ?? '—'),tryOnBadge(item)])} /><DataTable title="Variantes con stock" headers={['SKU','Producto','Almacén','Sucursal','Stock']} rows={(inventory.data ?? []).map(item => { const variant = variants.data?.find(value => value.idVariante === (item.variante?.idVariante ?? item.idVariante)); return [variant?.sku ?? item.variante?.sku ?? '—',variant?.producto?.nombre ?? '—',item.almacen?.nombre ?? '—',item.sucursal?.nombre ?? '—',String(item.stockDisponible)] })} /></div>
  </section>
}
