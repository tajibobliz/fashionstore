...
export default function Dashboard({ role }: { role: DashboardRole }) {
 ...
 <div className={styles.pageHeading}><div><p className={styles.kicker}>{role === 'ADMIN' ? 'Control de la plataforma' : role === 'ENCARGADO' ? 'Organiza tu negocio' : 'Tu espacio de atención'}</p><h1>{section ? current.label : `Dashboard ${role === 'ADMIN' ? 'ADMIN' : role === 'ENCARGADO' ? 'ENCARGADO' : 'CAJERO'}`}</h1><p>{role === 'ADMIN' ? 'Supervisa los usuarios, las prendas y las existencias del sistema.' : role === 'ENCARGADO' ? 'Prepara tu empresa, tus tiendas y tu equipo desde un solo espacio.' : 'Encuentra las prendas y consulta las existencias para atender a tus clientas.'}</p></div><button className={styles.refreshButton} onClick={reload} disabled={loading}><RefreshCw size={16} />Actualizar</button></div>
 ...
 <section className={styles.panel}><div className={styles.panelHeader}><div><h2>{role === 'ADMIN' ? 'Estado de la plataforma' : role === 'ENCARGADO' ? 'Prepara tu espacio de trabajo' : 'Antes de empezar tu turno'}</h2><p>{role === 'ADMIN' ? 'Empresas y su administración estarán disponibles próximamente.' : role === 'ENCARGADO' ? 'Registro de empresa, creación de tiendas, puntos de venta y cajas: próximamente.' : 'Asignación de caja y cobros: próximamente.'}</p></div><span className={styles.pendingBadge}>En desarrollo</span></div><Link to={`${base}/inventario`} className={styles.textLink}>Consultar inventario <ArrowRight size={16} /></Link></section>
 </>}
 </div>
}
